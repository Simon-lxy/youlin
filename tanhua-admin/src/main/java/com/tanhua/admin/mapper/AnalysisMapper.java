...
//    @Select("select sum(${column}) from tb_analysis where record_date between #{leDate} and  #{gtDate}")
    ...
    @Select("select sum(${column}) from tb_analysis where record_date >= #{leDate} and record_date <= #{gtDate}")
//    @Select("select sum(${column}) from tb_analysis where record_date between #{leDate} and  #{gtDate}")
    Integer sumUsers(@Param("column") String column, @Param("leDate") String leDate, @Param("gtDate") String gtDate);
